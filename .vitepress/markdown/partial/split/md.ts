...
const ignoreLines: Set<string> = new Set([...Array.from(dummyCodeWrappers)]);
...
export function splitMDContent(content: string, filename: string): MDChunk[] {
	...
	content.split('\n').forEach((line) => {
		...
		if (ignoreLines.has(trimmedLine)) return;
		...
		if (yamlLines) {
			if (trimmedLine === codeEndHeader) {
				...
				if (
					typeof data !== 'object' ||
					typeof (data as Record<string, unknown>).include !== 'string'
				) {
					// Return default
					const mergedCode = [
						yamlStartHeader,
				...
				if (!file)
					throw new Error(
						`Cannot locate included file "${include}" in "${filename}"`
					);
				...
				if (replacements) {
					if (!Array.isArray(replacements))
						throw new Error(
							`Bad replacements in include block in "${filename}"`
						);
		...
		if (trimmedLine.startsWith(inlineIncludeHeader)) {
			endContent();
			const include = trimmedLine
				.slice(inlineIncludeHeader.length)
				.replace(/`$/, '');
			const file = locateIncludedFile(include);
			if (!file)
				throw new Error(
	...
	// End
	endContent();
	if (yamlLines) throw new Error(`Incomplete yaml code block in "${filename}"`);
