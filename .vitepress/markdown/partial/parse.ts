			...
			return {
				chunk: {
					...
					content: replacements
						? replaceAllStrings(parsed.content, replacements)
						: parsed.content,
				},
...
function parseChunk(
	...
	const metadata = mergeImportedMetadata(
		parentFile,
		chunk.file.filename,
		parentMetadata,
		parsed.metadata
...
export function parseHTMLContent(src: string, filename: string): ParsedContent {
	...
	// Split to metadata/content
	let { content, metadata } = splitHTMLMetadata(
		cleanupMDContent(src),
		filename
	);
