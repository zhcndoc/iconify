...
export function splitMDMetadata(
	content: string,
	filename: string
): SplitMDMetadata {
	// Split lines
...
export function splitHTMLMetadata(
	content: string,
	_filename: string
): SplitMDMetadata {
	// Split lines
	...
	if (
		...
		(!firstLine.includes('type="text/yaml"') &&
			!firstLine.includes('lang="yaml"'))
	) {
		return {
			content,
		};
...
export function mergeMDMetadata(
	content: string,
	metadata?: MDMetaData
): string {
	// Do not add metadata if empty
	if (metadata && Object.keys(metadata).length) {
		return ['```yaml', stringify(metadata), '```', '', content].join('\n');
	}
