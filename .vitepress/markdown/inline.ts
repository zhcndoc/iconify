...
function wrapType(raw: string, _escaped: string): string {
	function split(type: string): string[] {
		...
		checkSpaces();
		if (type === '') return typesStart.concat(typesEnd);
		...
		for (let i = 0; i < midSeparators.length; i++) {
			...
			if (index !== -1) {
				...
				typesStart.push(separator);
				if (lastChunk.length) typesEnd = split(lastChunk).concat(typesEnd);
	...
	const childResults = types.map((type) => {
		type = type.replace(/</g, '&lt;').replace(/>/g, '&gt;');
		if (midSeparators.includes(type)) return type;

		return `<span class="hljs-inline-type hljs-linkable">${type}</span>`;
	});

	return `<span class="hljs-inline-type hljs-linkable">${childResults.join(
		''
	)}</span>`;
}
...
export function customInlineCodeMD(md: MarkdownRenderer) {
	...
	if (!oldParser) throw new Error('Missing default parser for code_inline');
	...
	md.renderer.rules.code_inline = (...params) => {
		...
		if (rawContent.slice(0, 1) !== '[') return oldParser(...params);

		// Get and remove [type]
		rawContent = rawContent.slice(1);
		const index = rawContent.indexOf(']');
		if (index === -1) return oldParser(...params);
		...
		switch (type) {
			...
			case 'attr':
				return (() => {
					...
					if (parts.length === 2) {
						...
						let value = parts[1];
						if (value === 'true' || value === 'false') valueType = 'literal';
						else if (value.match(/^-?[0-9.]+$/)) valueType = 'number';
			...
			case 'tag':
				...
				return wrap(
					'tag',
					`<span class="hljs-inline-tag hljs-linkable">${escapedContent}</span>`
				);
			...
			case 'packagist':
				return wrap(
					'npm',
					`<span class="hljs-inline-package hljs-linkable">${escapedContent}</span>`
				);
			...
			case 'str':
				return wrap(
					'str',
					`<span class="hljs-string">${escapeHtml(
						rawContent.replace(/^['"]?(.*)['"]$/, '$1')
					)}</span>`
				);
			...
			case 'css':
			case 'html':
				return `<span class="inline-code inline-code--${type}">${(() => {
					if (!hljs.getLanguage(type)) {
						throw new Error(`Bad language for inline code block: ${type}`);
					}
