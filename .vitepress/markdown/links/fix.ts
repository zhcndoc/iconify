...
function checkSimpleLink(href: string, env: MDEnv): string | undefined {
	...
	if (lastPartParts.length === 2 && lastPartParts.pop() === 'md') {
		...
		if (!env.cleanUrls) lastPartParts.push('html');
...
export function fixHTMLLink(
	...
	if (!target.startsWith('/')) {
		...
		if (!filename)
			throw new Error(
				'Cannot parse relative link when env.relativePath is missing'
			);
	...
	if (simpleLink) return simpleLink + params;
	...
	if (fixedLinksCache[target]) return fixedLinksCache[target] + params;
...
export function fixHTMLLinks(
	links: Record<string, string>,
	md: MarkdownRenderer,
	env: MDEnv
) {
	for (const key in links) links[key] = fixHTMLLink(links[key], md, env);
}
