...
export function parseYamlCode(code: string, filename: string) {
	...
	if (
		typeof data !== 'object' ||
		typeof (data as Record<string, unknown>).src !== 'string'
	) {
	...
	for (const key in defaultCodeSample) {
		...
		if (attr === 'extra') {
			...
			data.extra.forEach((source) => {
				if (
					...
					typeof (source as unknown as Record<string, unknown>).src !== 'string'
				) {
					throw new TypeError(
						`Invalid value type for "${attr}" in code block in ${filename}.`
					);
		...
		if (data[attr] === undefined) {
			...
			(data as unknown as Record<string, unknown>)[attr] = Array.isArray(
				defaultValue
			)
				? []
				: defaultValue;
		...
		switch (attr) {
			case 'demo':
				if (typeof data[attr] !== 'boolean' && typeof data[attr] !== 'string') {
					// Wrong type?
					throw new TypeError(
						`Invalid value type for "${attr}" in code block in ${filename}.`
					);
			...
			case 'replacements':
				...
				data.replacements.forEach((item) => {
					const search =
						(item as InlineCodeReplacement).search ||
						((item as InlineCodeReplacement2).code as unknown);
					const replace =
						(item as InlineCodeReplacement).replace ||
						((item as InlineCodeReplacement2).value as unknown);
					if (
					...
					replacements[search] = replaceAllStrings(
						replace,
						defaultReplacements
					);
				});
	...
	for (const key in data) {
		const attr = key as keyof CodeSample;
		if (defaultCodeSample[attr] === undefined) {
			throw new Error(
	...
	// Demo, before code
	if (data.demo && data.demoFirst) renderDemoSample(false);
	...
	// Demo
	if (data.demo && !data.demoFirst) renderDemoSample(true);
}
