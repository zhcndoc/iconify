...
export function mdConfig(md: MarkdownRenderer): MarkdownRenderer {
	...
	md.parse = (src, env) => {
		if (excludeTransform(env)) return oldParse.call(md, src, env);

		return oldParse.call(
			md,
			customMDParser(src, env?.relativePath ?? 'unknown'),
			env
		);
	};
	...
	md.render = (src, env) => {
		if (excludeTransform(env)) return oldRender.call(md, src, env);
