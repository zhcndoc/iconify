				...
				<div class="inner">
					<div class="code-blocks">
						<div class="code-block">
							<div
								class="code-block-content code-block-content--without-title code-block-content--without-hint"
							>
			...
			<dt>
				<div class="api-animation api-animation--0">
					<div class="animated-set animated-set--second animated-set--clouds">
						...
						<iconify-icon class="icon3" icon="line-md:cloud-down-twotone" />
						<p>API</p>
					</div>
					<div class="animated-arrows">
						<AnimationArrow class="animated-arrow" />
						<AnimationArrow class="animated-arrow animated-arrow--2" />
						<p>Icon Names</p>
					</div>
					<div class="animated-set animated-set--first animated-set--computers">
						...
						/>
						<iconify-icon class="iconify icon3" icon="line-md:laptop-twotone" />
						<p>Browser</p>
					</div>
				</div>
			</dt>
		...
		<dl>
			<dt>
				<div class="api-animation api-animation--1">
					...
					<div
						class="animated-set animated-set--second animated-set--computers"
					>
						<iconify-icon class="icon1" icon="line-md:laptop-twotone" />
			...
			<dt>
				<div class="code-blocks">
					...
					<div class="code-block">
						...
						>
							<code>
								&lt;iconify-icon icon="mdi:alert"&gt;<span class="hljs-comment"
									>#shadow-root</span
								>&lt;svg width="1em" height="1em" viewBox="0 0 24 24"
								<span class="hljs-comment">...</span>&gt;<span
									class="hljs-comment"
...
.icon-loading-process {
	...
	.api-animation {
		...
		$process-icon-set-width: calc(
			$process-icon-spacing * 2 + $icon-size - $process-icon-offset
		);
		...
		@each $counter in $api-animations {
			...
			@at-root {
				#{$prefix} {
					...
					.animated-arrows {
						p {
							animation-delay: calc(
								$process-text-arrows-delay + $process-arrow-delay +
					...
					.arrow-middle {
						animation-delay: calc(
							$process-arrow-delay + $api-animations-delay * $counter
						);
					}

					.arrow-sides {
						animation-delay: calc(
							$process-arrow-middle-duration + $process-arrow-delay +
								$api-animations-delay * $counter
						);
					}
					...
					.animated-arrow--2 {
						...
						.arrow-sides {
							animation-delay: calc(
								$process-arrow-middle-duration + $process-arrow-2-delay +
									$api-animations-delay * $counter
							);
						}
					}
					...
					.animated-set {
						p {
							animation-delay: calc(
								$process-text-set-delay + $api-animations-delay * $counter
							);
						}

						&--second p {
							animation-delay: calc(
								$set-2-animation-delay + $process-text-set-delay +
									$api-animations-delay * $counter
							);
						}
					}
					...
					@each $bp in $set-animation-breakpoints {
						@if ($counter > 0) {
							.il-md-delay-#{$bp} {
								animation-delay: calc(
									$icon-animation-step * $bp + $api-animations-delay * $counter
								);
							}
						}
