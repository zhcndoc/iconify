...
- 将 "DraftsIcon" 替换为你想要使用的组件名称（可自定义）。
...
所有图标组件均支持 `[prop]width` 和 `[prop]height` 属性。

值为字符串类型。

你无需同时设置这两个属性。如果只设置其中一个，另一个将根据图标的宽高比自动计算。
...
对于 SolidJS，回退图标组件发布在 `[npm]@iconify/css-solid` 包中。
