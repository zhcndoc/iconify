...
回退组件会检测过时的浏览器，从 [Iconify API](../../../api/index.md) 加载完整图标并渲染它，以替代小型 SVG。

回退图标是按需加载的，仅针对使用过时浏览器的访客。使用现代浏览器的用户不会从 API 加载任何内容。

对于 Svelte，回退图标组件发布在 `[npm]@iconify/css-svelte` 包中。
