...
# 在 Vue 中将图标用作 SVG + CSS

Iconify 提供了易于使用的组件，用于[将图标渲染为 SVG + CSS](../index.md)。
...
对于不支持 SVG + CSS 的浏览器，这些图标组件会使用一个回退组件。

回退组件会检测过时的浏览器，从 [Iconify API](../../../api/index.md) 加载完整图标，并渲染它来替代小型 SVG。

回退图标是按需加载的，仅针对使用过时浏览器的访客。使用现代浏览器的用户不会从 API 加载任何内容。

对于 Vue，回退图标组件发布在 `[npm]@iconify/css-vue` 包中。

未来，当 Safari 浏览器支持 `[prop]d` 属性的 `[func]path()` 函数时，将不再需要它，
但目前它已与图标捆绑在一起。
